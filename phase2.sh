...
# Wait for secret to be populatd
# while [ -z "$(kubectl get TargetKubeCluster proxy-cluster -o jsonpath='{.status.secretString}')" ]; do
#   sleep 1
# done
...
while ! kubectl api-resources | grep -q 'kubeclusters' && ! kubectl api-resources | grep -q 'apibindings'; do
  sleep 1
done
